...
char **get_random_paths(char **paths, int n, int m)
{
    char **random_paths = calloc(n, sizeof(char*));
    int i;
    pthread_mutex_lock(&mutex);  // in case picking the same image in a batch
    for(i = 0; i < n; ++i){
        int index = rand()%m;
        random_paths[i] = paths[index];
        //if(i == 0) printf("%s\n", paths[index]);
...
// added
char **get_random_paths_dac(class_paths *cls_paths, int n, int nthread) {
	char **random_paths = calloc(n, sizeof(char*));
	int i;
	pthread_mutex_lock(&mutex);
	for (i = 0; i < n; ++i) {
		int index = rand() % cls_paths[nthread*n + i].size;
		random_paths[i] = cls_paths[nthread*n + i].paths[index];
	}
	pthread_mutex_unlock(&mutex);
	return random_paths;
}
...
//user defined--lzy
data load_rgb_edge_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    char **random_paths = get_random_paths(paths, n, m);
    int i;
    data d = {0};
    ...
    d.y = make_matrix(n, 5*boxes);
    for(i = 0; i < n; ++i){
        image orig = load_image_edge(random_paths[i], 0, 0);
        image sized = make_image(w, h, orig.c);
        fill_image(sized, .5);
        ...
        random_distort_image_rgb_edge(sized, hue, saturation, exposure);
...
//user defined--lzy
data load_rggb_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    char **random_paths = get_random_paths(paths, n, m);
    int i;
    data d = {0};
    ...
    d.y = make_matrix(n, 5*boxes);
    for(i = 0; i < n; ++i){
        image orig = load_image_rggb(random_paths[i], 0, 0);
		image sized = make_image(w, h, orig.c);
        fill_image(sized, .5);
...
// added
data load_data_dac(int n, int nthread, class_paths *cls_paths, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure) {
	char **random_paths = get_random_paths_dac(cls_paths, n, nthread);
	int i;
	data d = { 0 };
	d.shallow = 0;
...
void *load_thread(void *ptr)
{
	...
	if (a.type == DAC_DATA) {
		*a.d = load_data_dac(a.n, a.nthread, a.cls_paths, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
    ...
    } else if (a.type == RGB_EDGE_DATA){
	*a.d = load_rgb_edge_data_detection(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
    } else if (a.type == RGGB_DATA){
        *a.d = load_rggb_data_detection(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
    }
    free(ptr);
    return 0;
}
